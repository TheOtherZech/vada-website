...
<div class="section" class:top={indent <= 0} class:has-children={children.length > 0}>
	...
	<div class="right">
		<span class:dot={depth > 0 && children.length > 0 == false}>
			<button on:click={navOrOpen} class="nav" class:highlight={active.element == target}>
				{@html title}
			</button>
		</span>
